...
# inference
mu_x = np.zeros((sensors_n,temp_orders))

eps_z = np.zeros((sensors_n,temp_orders))
eps_w = np.zeros((sensors_n,temp_orders))
xi_z = np.zeros((sensors_n,temp_orders))
xi_w = np.zeros((sensors_n,temp_orders))
pi_z = 100*np.ones((sensors_n,temp_orders))
pi_w = 1000*np.ones((sensors_n,temp_orders))
...
rho_history = np.zeros((2,iterations))                  # noisy version of the sensors

mu_x_history = np.zeros((iterations,sensors_n,temp_orders))
...
eta_mu_x = .01*np.ones((sensors_n,temp_orders))
...
#    vel[1] = x[i,1,1]                   # attach neuron to motor
    ...
    # vehicle 2
    vel[0] = f(sensor[1])                   # attach neuron to motor
    vel[1] = f(sensor[0])                   # attach neuron to motor
...
plt.figure(3)
plt.subplot(1,2,1)
plt.plot(range(iterations), vel_history[0,:], 'b', range(iterations), mu_x_history[:,0,1], 'r')
plt.subplot(1,2,2)
plt.plot(range(iterations), vel_history[1,:], 'b', range(iterations), mu_x_history[:,1,1], 'r')
